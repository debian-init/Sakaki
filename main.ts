...
// keep this out of the sakakiet itself, so as to prevent a message decryption/encryption loop across sakakiet restarts
...
// start a connection
	...
	store?.bind(sakaki.ev)

	// Pairing code for Web clients
	if (usePairingCode && !sakaki.authState.creds.registered) {
		// todo move to QR event
		const phoneNumber = await question('Please enter your phone number:\n')
		const code = await sakaki.requestPairingCode(phoneNumber)
	...
	const sendMessageWTyping = async(msg: AnyMessageContent, jid: string) => {
		...
		await sakaki.sendPresenceUpdate('composing', jid)
		await delay(2000)

		await sakaki.sendPresenceUpdate('paused', jid)

		await sakaki.sendMessage(jid, msg)
	}
